...
def _sample_chol_decomp_2d(m: int, n: int, vmin: float = -5, vmax: float = 5, include_zero: bool = False, use_gaussian: bool = False, device: torch.device = torch.device("cpu")):
    if use_gaussian:
        a = torch.randn(m, n, device=device) * (vmax - vmin)/6 + (vmax + vmin)/2
        b = torch.randn(m, n, device=device) * (vmax - vmin)/6 + (vmax + vmin)/2
        c = torch.randn(m, n, device=device) * (vmax - vmin)/6 + (vmax + vmin)/2
    else:
...
def get_m_samples(n_blocks: int, n_kernels: int, kernels_outside: bool = False, negative_experts: bool = False, vmin: float = -5, vmax: float = 5, include_zero: bool = True,
                  use_gaussian_chol: bool = False, device: torch.device = torch.device("cpu")):
    if kernels_outside:
        pad = 0.5
    else:
        pad = 0
    ...
    abc = _sample_chol_decomp_2d(n_blocks, n_kernels, vmin, vmax, include_zero, use_gaussian_chol, device).flatten(start_dim=1)
