...
def train_with_synth_data(n_kernels: int, run_cfg: Dict[str, Any]):
    # model = VariationalAutoencoder("manual_simple_ae.json")
    model = ResNetWeirdness(n_kernels=n_kernels, block_size=16)
    ...
    try:
        ...
        torch.save(model.state_dict(), f"{get_class_name(model)}_n_kernels_{n_kernels}_synth_data.pth")
...
if __name__ == "__main__":
    ...
    for i in range(3, 6):
        train_with_synth_data(n_kernels=i, run_cfg=cfg)
